      ...
      <table className="table table-bordered">
        ...
        <tbody>
          ...
          {cartItems.map(item => (
                <React.Fragment>
                                  <tr key={item.product}>
                <td className={`${rt1}`}><Link to={`/products/${item.product}`}><img height={'70px'} width={'70px'} src={item.image} alt={item.name} title={item.name} /></Link></td>
            <td className={`${rt1}`}><Link to={`/products/${item.product}`}>{item.name}</Link></td>
