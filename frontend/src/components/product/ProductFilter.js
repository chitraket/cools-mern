                ...
                {products && products.length !== 0 ?
                  <div className="row">
                    {products && products.map(product => (
                      <Product key={product._id} product={product} item={"product-layout  product-grid  col-lg-3 col-md-4 col-xs-6"} col={"mb_30"} addtocart={() => addToCart(product._id, 1, product.attribute && product.attribute[0].color && product.attribute[0].color.name)} user={user} isAuthenticated={isAuthenticated} favoriteHandler={(e) => favoriteHandler(e, product._id)} favoriteDeleteHandler={(e) => favoriteDeleteHandler(e, product._id)} />
                    ))}
                  </div>
